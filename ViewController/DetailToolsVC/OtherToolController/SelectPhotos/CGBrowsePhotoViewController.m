...
- (void)setupTitleWithPage:(NSInteger)page
{
    if (page < 1 || page > self.browsePhotoDataSource.count) {
        CGErrorLog(@"现在的索引为%li 索引应 大于0 小于%lu", (long)page, (long)self.browsePhotoDataSource.count);
        return ;
    }
    
    self.title  = [NSString stringWithFormat:@"%li/%lu", (long)page, (long)self.browsePhotoDataSource.count];
}
